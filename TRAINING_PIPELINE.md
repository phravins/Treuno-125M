...
```
Phase 1  Pretraining         100B tokens  ~4 days   4× A100 40GB  DeepSpeed ZeRO-2
Phase 2  Context Extension     5B tokens  ~12 hrs   4× A100 40GB  RoPE scaling
Phase 3  RAG-Aware SFT        20K pairs  ~2 hrs    2× A100 40GB  TRL SFTTrainer
Phase 4  Execution DPO        50K pairs  ~4 hrs    2× A100 40GB  TRL DPOTrainer
Phase 5  Uncertainty DPO      10K pairs  ~1.5 hrs  2× A100 40GB  TRL DPOTrainer
Phase 6  Weekly LoRA ΔUpdate   variable  ~2 hrs    2× A100 40GB  PEFT LoRA rank-16
```
...
| GitHub via RedPajama | 25B | Stars ≥ 100, CI passing |
...
Batch size:      512 sequences × 4,096 tokens = 2.1M tokens/step
Gradient clip:   1.0
Precision:       bfloat16
Attention:       FlashAttention-2
Distributed:     DeepSpeed ZeRO-2 across 4× A100 40GB
Checkpointing:   Every 10,000 steps
```

---

## Phase 2 — Context Window Extension
...
```
LR:      2e-5 (no warmup — continuing from Phase 1 checkpoint)
Context: 8,192
...
## Phase 3 — RAG-Aware Supervised Fine-Tuning
...
## Phase 4 — Execution-Driven DPO

**Goal:** Prefer code that runs over code that crashes — automatically, without human labels.
...
## Phase 6 — Continuous Weekly LoRA Update

**Goal:** Stay current with new library releases, API changes, and code patterns — without retraining.
...
Airflow DAG steps:
  1. Consume Kafka → raw JSONL (Parquet on S3/GCS)
  ...
  5. Launch LoRA rank-16 fine-tune (~2 hours on 2× A100)
  6. Shadow load → validate perplexity → hot-swap if improved
  7. Zero-downtime deploy via Kubernetes rolling update
```

**LoRA config:**
```
Rank (r):       16    (smaller than Phase 3 — optimized for speed)
Alpha:          32
...
|---|---|---|---|
| Phase 1 | 4× A100 40GB | 96h | ~$576 |
| Phase 2 | 4× A100 40GB | 12h | ~$72 |
| Phase 3 | 2× A100 40GB | 2h | ~$6 |
| Phase 4 | 2× A100 40GB | 4h | ~$12 |
| Phase 5 | 2× A100 40GB | 1.5h | ~$4.50 |
| Storage + egress | — | — | ~$100 |
| Buffer / reruns | — | — | ~$230 |
| **Total** | | | **~$1,000–$2,000** |
...
- **HumanEval** (164 Python problems) — primary pass@1
- **MBPP** (500 Python problems) — broad functional correctness
- **MultiPL-E** (HumanEval ported to 18 languages) — cross-language breadth
